...
![image-20250622214056613](./images//image-20250622214056613.png)

기존 레이어드 아키텍처에서 UI를 위처럼 관리하면 변경점이 너무 많아진다.

### 전담 프론트엔드 팀 추구

전담 프론트엔드 팀이 마이크로서비스 사용 회사에서 더일반적인 조직 패턴이다 왜?

이 팀에 대한 수요는, 전문가 부족, 일관성 추구, 기술적 문제라는 핵심 요소로 귀결되기 때문

* ui는 전문 기술이 필요함. 노하우도 필요함
* 일관성을 지켜야 유저가 불편함을 안느낌
* ui기술은 모놀리스가 아닌 방식으로 운영하기 어렵다. 

## 스트림 정렬 팀을 향해

- **기능 중심, 고객 중심으로 구성된 엔드투엔드 팀**
- 하나의 **"업무 흐름(stream of work)"**에 완전히 정렬된 팀
- **핸드오프 없이** 처음부터 끝까지 고객 가치 전달을 책임짐
- 팀의 목표는 **많은 기능 출시**가 아니라 **사용자 경험 개선**

왜 필요한데?

- 각 팀이 **고객 경험과 직접 연결**된 소프트웨어를 개발함
- KPI도 “기능 몇 개 만들었냐”가 아니라
   “**고객이 실제로 좋아졌는가**”로 평가됨
- 사용자와 가까울수록 팀이 **더 잘 기여**하고 있다는 걸 느낄 수 있음

전담 프론트엔드 팀이 필요한 이유는 무엇인가?

전담 프론트엔드 팀이 등장한 이유도 분명 존재함:

| 이유                              | 설명                                                         |
| --------------------------------- | ------------------------------------------------------------ |
| **전문성 확보**                   | 다양한 화면 단위의 복잡한 UI 구현 기술이 필요함 (예: React 성능 최적화 등) |
| **디자인 시스템/일관성**          | UI 패턴을 유지하고 공통 컴포넌트를 재사용하기 위해 중앙 관리가 편함 |
| **접근성, 반응형 등 기술적 고려** | 프론트엔드 고유의 품질 관리 요소 존재                        |

그렇다면 , 가능한 해결 방법

1. **스트림 정렬 팀 안에 프론트엔드 전문성을 내재화**
    → 모든 팀이 프론트 전문가를 일부 포함하거나 학습해서 **풀스택 지향**
2. **중앙 프론트엔드 팀은 플랫폼/도구/가이드 제공자로 전환**
    → 공통 컴포넌트나 디자인 시스템만 관리하고,
    실제 기능 개발은 각 스트림 정렬 팀이 담당
3. **디자인 시스템과 UI 원칙은 문서화하고 자동화된 테스트로 강제**
    → 사람이 일일이 컨트롤하지 않아도 **일관성 확보 가능**

## 14.4 모놀리식 프론트엔드

모놀리식 프론트엔드는 필요한 데이터를 얻거나 필요한 작업을 위해 백엔드 마이크로서비스를 호출하고 UI 자체에 정의되는 아키텍처이다.

![image-20250622224352574](./images//image-20250622224352574.png)

보통 이 상태에서는 인터페이스를 구성하는 컴포넌트를 만들고, JSON을 선택하며 백엔드와 상태 동기화 등을 처리한다 

### 적용 시점

단점이 있는데

1. 전담 프론트엔드 팀이 필요함. 여러 팀에서 이 모놀리식 프론트엔드의 책임을 나눌 수 없음. 
2. 다양한 종류의 기기에 맞게 응답을 조정할 수 있는 능력이 없음. 

## 14.5 마이크로 프론트엔드

독립적으로 제공 가능한 프론트엔드 애플리케이션들이 전체를 구성하게 만들 수 있다.

마이크로 프론트엔드 패턴은 계층화된 아키텍처에서 벗어나기 위해 엔드투엔드 스트림 정렬 팀을 도입하려는 경우 필수적이다. 또한 계층화된 아키텍처를 유지하고 싶지만 프론트엔드 기 능이 이제 너무 커져서 여러 전담 프론트엔드 팀이 필요한 상황에서는 유용할 수 있다.

## 페이지 기반 분해

- **UI를 여러 개의 웹 페이지 단위로 나누는 방식**
- 각 **페이지는 서로 다른 마이크로서비스가 제공**할 수 있음

- 각 페이지는 **다른 마이크로서비스에서 필요한 정보를 직접 가져옴**
   예) Album 페이지에서 **Inventory 마이크로서비스**를 호출하여 재고 표시

**장점**

1. **팀 단위 자율성**
   - 예: 앨범 팀은 **전체 페이지 렌더링 책임**
   - UI 변화가 사용자에게 어떤 영향을 주는지 **팀이 직접 파악 가능**
2. **기술적으로 단순함**
   - **브라우저 링크 클릭 → 새 페이지 요청** 방식
   - JavaScript SPA 프레임워크나 iFrame 없이도 구현 가능

## 제약

### 1. 다양한 기기와 사용자 제약

- 사용자는 데스크톱, 모바일, 태블릿 등 다양한 기기로 소프트웨어를 사용함
- 기기별로 해상도, 네트워크 상태, 입력 방식 등의 제약 조건이 존재
- 시각 장애인은 화면 판독기를, 거동이 불편한 사용자는 키보드 탐색을 주로 사용함

### 2. 동일한 서비스라도 UI는 조정이 필요함

- 기능은 같아도, 사용자 환경에 따라 UI를 조정해야 함
- 접근성을 고려하지 않으면 다음과 같은 문제가 발생할 수 있음:
  - 사용자 경험 저하
  - 특정 사용자의 서비스 접근 차단
  - 법적 제재 (예: 영국의 웹 접근성 법률)

### 3. 모바일 기기의 고유 제약

- 대역폭이 낮거나 불안정한 네트워크 환경
- 과도한 통신은 배터리 소모로 이어져 사용자 불만 유발
- 상호작용 방식 차이:
  - 모바일은 한 손, 주로 엄지손가락 사용
  - 태블릿은 우클릭 불가능
  - 일부 지역(남반구 등)은 SMS를 통한 서비스 사용이 일반적

### 4. 클라이언트 제약을 해결하는 일반적인 방법

- 클라이언트 측에서 데이터를 필터링하거나 호출을 집계함
  - 불필요한 데이터 제거
  - 여러 API 호출을 하나로 통합
...
- **중앙 집계 게이트웨이(Aggregation Gateway)**는 사용자 인터페이스(UI)와 여러 마이크로서비스 사이에 위치함
- UI의 **필요한 데이터만 필터링 및 집계하여 단일 응답으로 제공**
- UI가 여러 마이크로서비스를 직접 호출하는 대신, **게이트웨이에 한 번만 요청**

### 2. 예시 시나리오

- 고객 요약 화면을 표시할 때:
  - 고객 정보: `Customer 서비스`에서
  - 주문 목록: `Order 서비스`에서
  - → UI가 두 번 이상 호출해야 함
- 중앙 집계 게이트웨이를 사용하면:
  - UI는 게이트웨이에 **단일 요청**
  - 게이트웨이가 필요한 서비스들을 **대신 호출 → 필터링/집계 → 응답 구성**

### 3. 장점

- **호출 수 감소** 및 **불필요한 데이터 제거**
- **대역폭 절약**과 **지연 시간 감소**
- 배치 호출 가능 (ex. 주문 10건을 한 번에 요청)

### 4. 소유권 문제

- UI팀과 마이크로서비스팀이 **게이트웨이 변경을 두고 충돌**할 수 있음
- 어느 팀이 게이트웨이를 소유할지 불분명
- 결과적으로 **전담 팀이 게이트웨이를 소유**하게 되는 경우 많음
- → 다시 **계층형 아키텍처의 병목** 발생

### 5. 다양한 클라이언트 문제

- 모바일과 웹은 **요구하는 데이터가 다름**
  - 웹: 상세 정보 + 요약 정보
  - 모바일: 간단한 요약 정보
- 하나의 게이트웨이에 너무 많은 요구가 쌓이면 **유지보수와 배포 병목 초래**

### 6. API 게이트웨이 제품 사용 시 주의점

- 인증, 속도 제한, 라우팅 등은 **제품 사용**이 적절
- **호출 집계와 필터링 로직까지 제품에 의존하면 단점 존재**
  - 도구가 제공하는 DSL, 제한된 언어, 폐쇄적 구성
  - 기능 이관이 어려워짐 (ex. 공급업체 종속성)

### 7. 실무상의 문제점

- 게이트웨이 기능이 복잡해져 전담 팀이 필요
- 각 팀이 연쇄적으로 수정해야 하는 구조가 되면 **개발 속도 느려짐**
  - 예: 프론트엔드 팀, 게이트웨이 팀, 서비스 팀 모두 수정해야 릴리스 가능

### 8. 적용 시점

- UI와 백엔드를 **하나의 팀**이 소유할 경우엔 괜찮음
  - UI 간 요구사항이 유사하다면 굳이 필요 없을 수도 있음
- 하지만 여러 팀이 협업해야 한다면:
  - **중앙 게이트웨이는 조율의 병목이 되므로 신중히 고려**
  - **필터링/집계 로직은 BFF 등으로 분산**하는 것이 바람직

## 14.10 프론트엔드를 위한 백엔드(BFF)

### 1. 개념

- BFF는 **특정 사용자 인터페이스(UI)** 전용으로 설계된 서버 사이드 컴포넌트
- 중앙 집계 게이트웨이와 달리, **"모든 UI용"이 아닌 "특정 UI용"**
- UI와 강하게 결합되어 있으며 **UI 팀이 직접 소유하고 관리**하는 것이 일반적
- 마치 UI가 두 부분(클라이언트와 BFF)으로 나뉜 것처럼 구성됨

### 2. 장점

- 중앙 게이트웨이처럼 **병목이나 소유권 분쟁이 덜함**
- 각 UI별로 **특화된 API 설계 가능**
- **릴리스 주기 통합이 쉬움** (UI와 BFF를 같은 팀이 관리)
- 모바일 등 특화된 클라이언트 요구에 유연하게 대응 가능

### 3. BFF 개수에 대한 전략

- **엄격한 방식**: 클라이언트 유형별로 BFF를 각각 둠 (ex. iOS BFF, Android BFF)
- **완화된 방식**: 기능이 유사한 클라이언트(iOS/Android)에서 **하나의 BFF 공유**
  - 같은 팀이 관리하는 경우는 공유 가능
  - 팀이 나뉘어 있다면 **분리 유지 권장**
- 조직 구조가 BFF 전략 결정에 큰 영향을 줌 (콘웨이의 법칙)

### 4. 코드 중복과 재사용 문제

- BFF를 나누면 중복 코드 발생 가능
- 중복 해결 방법
  1. **공통 라이브러리** 추출
     - 비용이 적지만 서비스 간 결합 위험
  2. **공통 마이크로서비스**로 기능 추출
     - 예: 위시리스트 마이크로서비스
     - 기능이 반복된다면 새로운 서비스로 만드는 것이 더 나을 수 있음

### 5. 데스크톱 및 외부 대상 BFF

- 데스크톱 웹도 BFF가 **캐싱, 외부 호출 캡슐화** 측면에서 유용할 수 있음
- BFF는 제삼자 API를 래핑하는 용도로도 사용 가능
  - 예: 로열티 정산을 위한 외부 파트너용 BFF
- 이 경우 UI가 없지만 **패턴은 동일하게 적용됨**

### 6. 적용 시점

- **모바일 앱 또는 외부 파트너와의 통합**이 있는 경우 BFF를 적극 도입
- 웹 UI만 있는 경우에는 **서버 측 집계가 클 때만 사용**
- **UI와 백엔드 개발자가 분리된 경우**에는 BFF 도입이 매우 효과적
- BFF는 **관심사 분리**, **개발 속도**, **확장성** 측면에서 장점이 많음
...
- **GraphQL**은 클라이언트가 원하는 데이터를 명확하게 쿼리할 수 있는 **쿼리 언어**
- SQL처럼 **동적으로 필요한 필드만 지정 가능**
- REST API처럼 고정된 리소스가 아니라, **필요한 데이터만 조합해서 요청 가능**
...
- 위 쿼리는 주문 ID 123번의 `date`, `total`, `status`, `delivery` 정보를 요청
- REST 방식이라면 여러 번 API 호출이 필요할 수 있으나, GraphQL은 **한 번의 요청으로 해결**

### 3. 기술적 구성 요소

- **타입 정의**: 쿼리 가능한 데이터 타입과 필드를 명시적으로 정의
- **리졸버(resolver)**:
  - 서버 측 함수
  - GraphQL 쿼리를 실제 서비스 호출로 변환해 데이터 반환
  - 마이크로서비스 구조에서도 리졸버는 각 서비스 호출을 담당

### 4. 장점

- **불필요한 데이터 제거**: 클라이언트가 필요한 데이터만 선택 가능
- **왕복 횟수 감소**: 여러 API 호출을 한 번의 쿼리로 처리
- **유연한 확장성**:
  - 쿼리에서 필드만 추가하거나 제거하면 결과가 달라짐
  - **서버 변경 없이도 클라이언트 단에서 구성 조정 가능**
- **BFF 구현에도 적합**:
  - 특정 UI 요구사항에 맞게 데이터를 구성할 수 있음
  - 여러 UI가 **공통 GraphQL API**를 공유해도 충돌 위험이 적음

### 5. 주의점

- **완전한 무변경은 아님**:
  - 새 필드나 타입을 추가하려면 여전히 서버 측 변경 필요
- **GraphQL 서버도 팀 경계에 맞춰 구성하는 것이 바람직**
- **유지보수, 보안, 캐싱 전략 등 고려 필요** (5장에서 언급됨)

## 14.12 하이브리드 방식

### 1. 조합 가능성

- 하나의 접근 방식만 고수할 필요 없음
  - 예: **웹에는 위젯 기반 UI 구성**, **모바일에는 BFF 적용**
- 핵심은 **기능 일관성 유지**:
  - 예: 음악 주문, 고객 정보 변경 등 도메인 로직은 **서비스 내부에 집중**

### 2. 중간 계층의 과도한 책임 주의

- BFF, 게이트웨이, GraphQL 등 중간 계층에 너무 많은 로직이 모이면 복잡도 증가
- **적절한 역할 분리와 책임 최소화**가 중요

### 요약 비교

| 항목             | GraphQL                               | REST + BFF                          |
| ---------------- | ------------------------------------- | ----------------------------------- |
| 데이터 요청 방식 | 원하는 필드를 선택하여 요청 가능      | 고정된 엔드포인트 설계 필요         |
| 서버 변경 여부   | 대부분 필요 없음 (타입 정의로 충분)   | 요청 요구사항 변경 시 BFF 수정 필요 |
| 중복 호출 방지   | 쿼리 하나로 여러 자원 접근 가능       | 여러 REST 호출 필요 가능성 있음     |
| 팀 경계 고려     | 리졸버 및 스키마를 경계별로 분리 가능 | BFF 역시 팀 단위로 구성 가능        |
