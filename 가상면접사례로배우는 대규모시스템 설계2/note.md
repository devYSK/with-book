...
* block 저장소
  * 원시(raw) 블록을 서버에 volume 형태로 제공함. 가장 유연하고 융통성이 높음.
  * 네트워크나 FC(Fibre Channel) 프로토콜 등을 통해 연결 가능함. 
* file 저장소
  * 블록 저장소 위에 구현되며, 파일과 디렉터리를 손쉽게 다루도록 추상화를 제공함
  * NFS 같은 네트워크 프로토콜을 사용하여 여러 서버에 동시에 붙일수도 있음. 
  * 보통 조직 구성원에 공유하는 솔루션으로 사용 
* object 저장소
  * 실시간으로 갱신할 필요가 없는 cold 데이터 보관에 초점을 두며 아카이브나 백업에 주로 쓰임
  * 모든 데이터를 수평적 구조 내에 객체로 보관하며 계층적 디렉터리는 제공하지 않음?
  * RESTful API를 통해 데이터 접근을 제공하며 다른 유형의 저장소에 비해 상대적으로 느림

<img src="./images//image-20250217185900416.png" width = 650>

| 저장소 유형                   | 블록 저장소                                                  | 파일 저장소                   | 객체 저장소                             |
| ----------------------------- | ------------------------------------------------------------ | ----------------------------- | --------------------------------------- |
| **저장된 내용의 변경 가능성** | Y                                                            | Y                             | N (직접 변경 불가, 객체 버전 추가 가능) |
| **비용**                      | 고                                                           | 중~고                         | 저                                      |
| **성능**                      | 중~고 혹은 최상                                              | 중~고                         | 저~중                                   |
| **데이터 일관성**             | 강력                                                         | 강력                          | 강력                                    |
| **데이터 접근 방식**          | SAS/iSCSI/FC                                                 | 표준 파일 접근, CIFS/SMB, NFS | RESTful API                             |
| **규모 확장성**               | 중                                                           | 고                            | 최상                                    |
| **적합한 응용**               | 가상 머신(VM), 데이터베이스 등 높은 성능이 필요한 애플리케이션 | 범용적 파일 시스템 접근       | 이진 데이터, 구조화되지 않은 데이터     |

### 용어 정리

* bucket : 객체를 보관하는 논리적 컨테이너. 버킷명은 전역 유니크 해야한다.
* object : 객체는 버킷에 저장하는 개별 데이터를 말한다. 객체는 데이터와 메타데이터를 갖는다. 
* versioning : 한 객체의 여러 버전을 같은 버킷안에 둘 수 있도록 하는 기능이다.
* URI : 해당 API URI를 통해 고유하게 식별가능하며 객체에 접근 가능하다
* SLA : 서비스 제공자와 클라이언트 사이에 맺어지는 계약

## 1단계 문제 이해 및 설계 범위 확정

* 어떤 기능을 제공해야 하나요?
  * 버킷 생성, 객체 업로드 및 다운로드, 객체 버전, 버킷 내 객체 목록 출력 기능
* 데이터의 크기는 어느정도인가요?
  * 수 GB 이상, 수 KB 를 효율적으로 저장할 수 있어야 함
* 매년 추가되는 데이터는 얼마인가요?
  * 100 페타바이트 
* 99.9999%의 데이터 내구성과 99.99%의 서비스 가용성을 보장해야 함

### 대략적 규모 추정

객체 저장소는 디스크 용량이나 초당 디스크 IO가 병목이 될 가능성이 높다. 

* 디스크 용량 : 객체 크기의 분포도를 다음처럼 가정
  * 객체 가운데 20%는 크기가 1MB 미만
  * 60% 정도의 객체는 1MB ~ 64MB정도 중간 크기
  * 나머지 20%는 64MB 이상 대형 객체 
* IOPS : SATA 인터페이스, 7200rpm 하드디스크 초당 100 ~ 150회 임의 데이터 탐색 지원

## 2단계 개략적 설계안 제시 및 동의 구하기

객체 저장소의 속성에 대해 알아야 한다

* 불변성 : 객체 저장소에 보관되는 객체들은 변경이 불가능하며, 삭제한 다음 다른 객체로 대체해야 한다
* 키-값 저장소 : 해당 객체의 URI를 사용하여 데이터를 가져온다. 
* 저장은 1회, 읽기는 여러번 : 95%이상의 요청이 읽기 요청이다

객체 저장소의 설계 철학은 UNIX 파일 시스템 설계 철학과 비슷하다.

파일의 이름과 데이터는 같은곳에 저장되지 않는다. 파일 이름은 inode라는 자료 구조에 보관되고 파일의 데이터는 디스크의 다른 위치에 들어간다. 아이노드에는 파일 블록 포인터 목록이 저장된다.

객체 저장소의 동작 방식도 비슷하다

메타 데이터 저장소는 아이 노드에 해당하고, 객체 데이터는 하드 디스크에 있다.

메타 데이터 저장소에는 파일 블록 포인터 대신 네트워크를 통해 데이터 저장소에 보관된 객체를 요청하는데 필요한 식별자가 저장되어 있다.

<img src="./images//image-20250217194051844.png" width = 450>

### 개략적 설계안

![image-20250217194131505](./images//image-20250217194131505.png)

* 로드밸런서 : API 요청을 분산하는 역할
* IAM 서비스 : 인증, 권한부여, 접근제어를 맡아 처리
* 데이터 저장소 : 데이터를 보관하고 필요할때마다 읽어가는 장소. 모든 데이터 관련 연산은 객체 ID를 통함

### 객체 업로드 과정

![image-20250217195423953](./images//image-20250217195423953.png)

### 객체 다운로드

![image-20250217195454205](./images//image-20250217195454205.png)

## 3단계 상세 설계

### 데이터 저장소

데이터 저장소는 세가지 주요 컴포넌트로 구성된다.

<img src="./images//image-20250217195555815.png" width = 450 height = 350>

### 데이터 라우팅 서비스

데이터 라우팅 서비스는 데이터 노드 클러스터에 접근하기 위한 RESTful 또는 gRPC 서비스를 제공한다 

라우팅 서비스는 규모를 쉽게 확장할 수 있는 stateless 서비스다.

* 배치 서비스(placement service)를 호출하여 데이터를 저장할 최적의 데이터 노드 판단
* 데이터 노드에서 데이터를 읽어 API 서비스에 반환
* 데이터 노드에 데이터 기록

### 배치 서비스(placement service)

 배치 서비스는 어느 데이터 노드에 데이터를 저장(배치)할지 결정하는 역할을 담당한다.

데이터 노드는 프라이머리와 세컨더리 노드가 있다. 배치 서비스는 내부적으로 가상 클러스터 지도를 유지하는데, 이 지도에는 클러스터의 물리적 형상 정보가 보관된다. 이 지도를 이용해서 데이터 레플리카가 물리적으로 다른 위치에 놓이도록 한다.

![image-20250217200201644](./images//image-20250217200201644.png)

배치 서비스는 모든 데이터노드와 지속적으로 하트비트 메시지를 주고받으며 상태를 모니터링 한다. 

5개에서 7개 노드를 갖는 배치 서비스 클러스터를 paxos나 raft 같은 합의 프로토콜을 사용하여 구축하는것이 권장된다 

* *Paxos**는 **Leslie Lamport**가 제안한 **분산 합의 알고리즘
* **Raft**는 **Paxos보다 이해하고 구현하기 쉬운 합의 알고리즘**

7개 노드라면, 최대 3개까지 노드 장애는 감내할 수 있다.

### 데이터 노드

데이터 노드는 실체 객체 데이터가 보관되는 곳이다. 여러 노드에 데이터를 복제한다. 이를 다중화 그룹이라고 한다.

각 데이터 노드에는 주기적으로 하트비트 메시지를 보내는 서비스 데몬이 돈다. 메시지에는 다음과 같은 정보가 있다.

* 해당 데이터 노드에 부착된 디스크 드라이브 HDD/SSD의 수
* 각 드라이브 저장된 데이터 양

### 데이터 저장 흐름

![image-20250217201346849](./images//image-20250217201346849.png)

1. API 서비스는 객체 데이터를 데이터 저장소로 포워딩
2. 데이터 라우팅 서비스는 해당 객체에 UUID를 할당하고 배치 서비스에 해당 객체를 보관할 데이터 노드를 질의 하고, 배치 서비스는 가상 클러스터 맵을 확인하여 데이터를 보관할 주 데이터 노드를 보관한다
3. 데이터 라우팅 서비스는 저장할 데이터를 UUID와 함께 주 데이터 노드에 직접 전송한다. 
4. 주 데이터 노드는 자기 노드에 저장하는 한편 두 개의 부 데이터 노드에 다중화 하고, 주 데이터 노드는 다중화 성공시 데이터 라우팅 서비스에 응답을 보낸다
5. 객체의 UUID인 객체 아이디를 API에 반환한다.

### 데이터는 어떻게 저장되는가

각 객체를 개별 파일로 저장시 작은 파일이 많아지면 성능이 떨어진다. 작은 파일이 많아지면 두가지 문제가 생긴다.

1. 낭비되는 블록 수가 늘어난다. 디스크 블록의 크기는 전부 같으며 볼륨 초기화시 결정되는데 보통 4KB이다. 1KB를 저장해도 4KB를 쓴다. 즉 낭비가 심해질 수 있다.
2. 시스템의 inode 용량 한계를 초과할 수 있다. 작은 파일의 수가 수백만에 달하게 되면 아이노드가 전부 소진될 가능성이 생긴다. 

이 문제들은 작은 객체들을 큰 파일 하나로 모아서 해결할 수 있다. WAL와 같이 객체를 저장할 때 이미 존재하는 파일에 추가하는 방식이다. 용량 임계치에 도달한 파일은 읽기 전용 파일로 변경하고 새로운 파일로 만든다.(보통 수 GB)

![image-20250217203812894](./images//image-20250217203812894.png)

읽기-쓰기 파일에 대한 쓰기 연산은 순차적으로 이루어져야 한다.

객체는 파일에 일렬로 저장되는데 이 레이아웃을 유지하려면 여러 CPU 코어가 쓰기 연산을 병렬로 진행하더라도 객체 내용이 뒤섞이는 일은 업어야 한다. 즉 파일에 객체를 기록하기 위해서 자기 순서를 기다려야 하며 멀티코어 시스템에서는 대역폭이 심하게 줄어든다.

이를 해결하려면 서버에 오는 요청을 처리하는 코어별로 전담 읽기-쓰기 파일을 두어야 한다.

### 객체 소재 확인

각각 데이터 파일 내에 많은 작은 객체가 들어있다면, 다음 정보를 가지고 파일을 찾는다

* 객체가 보관된 데이터 파일
* 데이터 파일 내 객체 오프셋(offset)
* 객체 크기

object_id, file_name, start_offset, object_size 같은 메타데이터 값들은 보통 관계형 베이터베이스에 저장하는것이 좋다. 읽기 연산 성능이 좋기 때문이다.

하나의 거대 클러스터가 답이 아니다. 데이터 노드에 저장되는 위치 데이터를 다른 데이터 노드와 공유할 필요가 없으므로, 노드마다 파일기반  SQLite로 관계형 데이터베이스를 사용하면 된다

### 개선된 데이터 저장 흐름

![image-20250217204134000](./images//image-20250217204134000.png)

### 소거코드(erasure coding를 이용한 99.9999% 데이터 내구성 달성

소거코드는 데이터를 작은 단위로 분할하여 다른 서버에 배치하면서, 가운데 일부가 소실되었을 때 복구하기 위한 parity라는 정보를 만들어 중복을 확보하는 것이다. 장애가 생기면 남은 데이터와 패리티를 조합하여 소실된 부분을 복구한다

* XOR 연산의 성질을 이용해서 데이터를 복구한다.

1. ﻿﻿﻿데이터를 네 개의 같은 크기 단위로 분할한다(dl, d2, d3, d4).
2. ﻿﻿﻿수학 공식을 사용하여(2) 패리티 p1, p2를 계산한다. 아주 단순한 예제를 들 자면, pI=d1+2xd2-d3+4Xd4, p2=-d1+5xd2+d3-3xd4124와 같이 계산할 수 있다.
3. ﻿﻿﻿데이터 d3와 d4가 노드 장애로 소실되었다고 하자.
4. ﻿﻿﻿남은 값 dl, d2, pl, p2와 패리티 계산에 쓰인 수식을 결합하면 d3와 d4를 복원할 수 있다.

소거코드 사용시 필요한 오버헤드는 50%이며, 3중 복제 다중화를 채택하는 경우 200%이다.

소거 코드는 99.11-nine의 내구성을 달성할 수 있다. 

<img src="./images//image-20250217220445235.png" width = 400>

이 둘을 비교하자면

| 항목              | 다중화 (Replication)                    | 소거 코드 (Erasure Code)                                     |
| ----------------- | --------------------------------------- | ------------------------------------------------------------ |
| **내구성**        | 99.9999% (3중 복제)                     | 99.999999999% (8+4 소거 코드) → 소거 코드가 우월             |
| **저장소 효율성** | 200% 저장 용량 오버헤드                 | 50% 저장 용량 오버헤드 → 소거 코드가 우월                    |
| **계산 자원**     | 추가 계산 필요 없음 → 다중화가 우월     | 패리티 계산에 많은 계산 자원 소모                            |
| **쓰기 성능**     | 데이터 복제만 수행 → 다중화가 우월      | 패리티 계산이 필요하여 응답 지연 증가                        |
| **읽기 성능**     | 정상 노드에서 직접 읽기 → 다중화가 우월 | 여러 노드에서 데이터를 조합해야 하므로 지연 증가 (특히 장애 발생 시) |



응답 지연이 중요하면 다중화 방안이 좋고, 저장소 비용이 중요한 애플리케이션은 소거 코드가 좋다. 



### 큰 파일의 업로드 성능 최적화

GB 이상인 데이터들은 작게 쪼갠다음 독립적으로 업로드 하고, 저장소는 그 조각을 모아서 원본 객체를 복원한다.

* 멀티파트 업로드

<img src="./images//image-20250217223452843.png" width = 350>

* 클라이언트는 각 파트를 ETag와 함께 데이터 저장소에 올린다
* 조각 하나가 업로드 될때마다 ETag를 반환한다. Etag는 해당 조각에 대한 MD5 해시 체크섬이다. 멀티파트 업로드가 정상적으로 되었는지 검사할때 이용한다.
* 모든 조각 업로드 종료시, uploadId, 조각 번호 목록, Etag 목록이 포함되어야 한다. 

객체 조립이 끝난 뒤 조각들은 더이상 쓸모가 없으므로, 쓰레기 수집 프로세스를 구현하여 조각을 지워야 한다. 
...
* 사용자는 경기에서 승리하면 포인트를 얻는다. (1점씩)
* 매달 새로운 토너먼트 시작 시마다 새로운 순위표를 만든다.
* 토너먼트 참가 사용자는 DAU 500만, MAU 2500만
* 하루 평균 10경기
* 동점일 경우 동일
* 점수 업데이트는 실시간으로 순위표에 반영된다 
* 상위 10명의 플레이어와, 특정 사용자의 순위를 표시해야 한다.
  * 보너스로 어떤 사용자보다 4순위 위와 아래에 있는 사용자를 표시한다.

### 개략적 규모 추정

24시간동안 고르게 분포한다고 가정하면, DAU 500만 / 86400 = ~50(근사값) = 초당 50 tps

여기다 최대 부하는 평균의 5배로 가정하여 250tps를 감당할 수 있어야 한다.

* 점수 획득 QPS 계산 :  하루 평균 10개의 게임이면 250 x 10 = 2500 tps를 견뎌야 한다.

* 상위 10명 순위표 TPS 계산 : 각 사용자가 하루에 한 번 게임을 열고, 상위 10명 순위표는 처음에만 연다고 가정하면 QPS = 50

## 2단계 : 개략적 설계안 제시 및 동의구하기

API , 아키텍처, 데이터 모델 설계가 필요하다
...
> 클라이언트가 점수를 정하여 순위를 정하는것은 안된다.
>
> 사용자가 프록시 서버를 통해 점수를 마음대로 바꾸는 중간자 공격을 할 수 있기 때문에 보안상 안전하지 않다. 
>
> 따라서 점수는 서버가 설정해야 한다. 

#### 게임 서비스와 순위표 서버 사이에 메시지 큐가 필요한가?

게임 점수가 다른곳에서도 이용되거나 여러 기능을 지원해야 한다면 카프카에 데이터를 넣는 것이 합리적일 수 있다.

* 순위표 서비스, 분석 서비스, 푸시 알림 서비스 등..

점수가 바뀌었음을 알려야 하는 순번제 게임이나 멀티플레이 게임의 경우 더 그렇다.

면접관이 명시적으로 요청한 사항은 아니므로, 본 설계안에는 메시지 큐를 포함시키지 않아도 된다.
...
규모 확장성이 중요하지 않고, 사용자 수가 많지 않다면 RDS를 이용하면 된다.

월별 순위표 테이블에 user_id와 점수를 나타내는 score를 이용하면 된다. 

레코드가 수백만개 가 되면 성능이 너무 나빠진다.

### 레디스

sorted set을 이용하면 특정 스코어로 정렬된 요소들을 찾기 좋다. 

**해시 테이블에 값 -> 스코어**를, **스킵 리스트에 스코어를 저장**한다

* **HashMap(dict)**으로 빠르게 값 찾고,
* **Skip List**로 정렬 및 순위 검색을 빠르게 처리함. 

솔티드셋은 삽입 갱신 연산시 모든 원소가 올바른 위치에 자동으로 배치되며, 원소 추가 및 검색 연산 시간 복잡도가 O(log(n))이므로 RDB보다 성능이 좋다. 

레디스 연산들을 살펴보자

* ZADD : O(log(n)) : 기종에 없던 사용자를 집합에 삽입. 기존 사용자의 경우 점수 업데이트
* ZINCRBY: O(log(n)) 사용자 점수를 지정된 값 만큼 증가. 

* ZRANGE / ZREVRANGE: O(log(n) + m) : 특정 범위에 드는 사용자들을 가져옴 
  * m은 가져올 항목 수, n은 정렬 집합의 크기 
  * ZREVRANGE는 지원이 중단됐으므로 ZRANGE leaderboard 0 9 REV WITHSCORES 사용 
* ZRANK / ZREVRANK : O(log(n))오름차순/내림차순 정렬 시 특정 사용자의 위치를 가져온다.



1. 사용자가 점수를 획득한 경우
   * ZINCRBY로 사용자 점수를 증가시킨다
   * 
2. 사용자가 순위표 상위 10명을 조회하는 경우
   * ZREVRANGE : leaderboard_feb_2021 0 9 WITH_SCORES
   * ZRANGE leaderboard 0 9 REV WITHSCORES 사용. 
3. 사용자가 자기 순위 조회하는 경우
   * ZREVRANK leaderboard_feb_2021 '유저명'
4. 특정 사용자 기준으로 일정 범위 내 사용자 질의하는 경우
   * 예를들어 malow007의 랭크가 361이고, 전 후 4명씩 가져오려면
   * ZRANGE leaderboard 357 365

#### 레디스 저장소 요구사항

2500만명과 점수 모두 저장해야 한다. 

* id 가 24자 문자열, 점수가 16비트 정수라고 한다면 한 항목당 26바이트

26바이트 * 2500만 = 650MB 저장공간이 필요하다. 

다음으로 CPU 및 I/O 사용량 계산

* 갱신연산의 최대 QPS는 2500/초 
* 반면 레디스는 인스턴스와 토폴로지 모델에 따라 최소 10만 qps 이상 견딤

레디스에 읽기 사본을 두고 RDB에도 백업용으로 저장한다. 
...
이름 및 프로필 이미지 등 사용자 정보는 MySQL에 둔다. 너무 느리면 redis에도 프로필을 캐시한다

#### 클라우드 서비스

AWS API 게이트웨이와 AWS 람다를 이용해 실행한다.

<img src="./images//image-20250218001845735.png" width = 550>

### 레디스 규모 확장

5백만 DAU 정도라면 한 대의 레디스 캐시 서버로도 충분

5억 DAU라면, 저장 용량은 65GB, 250,000QPS(2,500 x 100) 처리가 필요. 이경우 샤딩 필요

#### 데이터 샤딩 방안 1 - 고정 파티션

고정 파티션은 순위표 등장하는 점수의 범위에 따라 파티션을 나누는 방안.

특정 범위별로 샤드에 나눠 저장하고, 애플리케이션에서 위치에 맞게 조회한다

#### 데이터 샤딩 방안 2 - 해시 파티션

레디스 클러스터를 이용한다. 안정해시는 사용하지 않지만, 각 키가 특정 해시 슬롯에 속하도록 함.

총 16384의 해시 슬롯이 있으며 CRC16(key) % 16384의 연산을 수행하여 어떤 키가 어느 슬롯에 속하는지 계산. 

<img src="./images//image-20250218002137620.png" width = 550>

단 샤드에서 상위 10명을 조회하기엔 까다롭다. 

모든 샤드에서 상위 10명을 받아 애플리케이션 내에서 다시 정렬하는 분산-수집 접근법을 사용해야 한다.

병렬로 조회하더라도 이 방법은 문제가 있다.

- ﻿﻿상위 k개의 결과를 반환해야 하는 경우(k가 아주 큰 값이라고 하자), 각 샤 드에서 많은 데이터를 읽고 또 정렬해야 하므로 지연 시간이 늘어난다.
- ﻿﻿가장 느린 파티션에서 데이터를 다 읽고 나서야 질의 결과를 계산할 수 있으 므로 지연 시간이 길어진다.
...
* 갱신을 하더라도 고정 파티션이 더 합리적.

#### 레디스 노드 크기 조정

쓰기 작업이 많다면 많은 메모리가 필요하며, 장애에 대비에 스냅숏을 할당할 때 메모리를 두 배 더 크게 잡아야 한다.

* redis-benchmark로 테스트 가능. 각 명령어의 QPS 확인 가능

### 대안 NoSQL

쓰기 연산에 최적화 되어 있으며, 같은 파티션 내의 항목을 저무에 따라 효율적으로 정렬 가능해야 한다.

후보로는 DynamoDB, 카산드라, MongoDB가 있다.

다이나모디비를 사용하고, 다이나모디비는 안정 해시를 사용하여 여러 노드에 데이터를 분산한다.

최근 한 달치 데이터가 동일한 파티션에 저장되면 핫 파티션 문제가 생기므로 

데이터를 n개 파티션으로 분할하고, 파티션 번호를 파티션에 키에 추가할 수도 있다. 

* user_id % number_of_partitions

하지만 읽기 쓰기 작업 모두를 복잡하게 만든다.

대안으로, 파티션 키는 game_name#{year-month}#p{partition_number}로 둔다.

![image-20250218002910967](./images//image-20250218002910967.png)

같은 파티션 내 데이터는 전부 점수 기준으로 정렬된 n개의 파티션이 만들어지며, 

상위 10개의 사용자를 가져오려면 분산-수집을 이용해서 각 파티션에서 상위 10개를 가져온다음 애플리케이션단에서 정렬한다.

*  그러나 앞서 언급한 레디스 파티션 기법처럼 사용자의 상대적 순위를 쉽게 정할 수 없다. 

* 하지만 사용자 위치의 백분위수를 구하는 것은 가능하며, 그것도 충분히 괜찮은 방법일 수 있다. 
  * 1,200,001로 정 확한 순위를 표시하는 것보다 상위 10~20%에 속한다고 말하는 것이 더 나을 수 있다는 것이다. 규모가 충분히 커서 샤딩이 필요한 상황이라면 모든 샤드의 점수 분포는 거의 같다고 가정할 수 있다



## 4단계 : 마무리

레디스에 샤딩하여 5억 DAU도 지원할 수 있도록 규모를 확장하는 법도 찾았다.

다음 방법도 고민해보자

### 더 빠른 조회 및 동점사 순위 판정 방안

레디스 해시(Redis Hash)를 사용하면 문자열 필드와 값 사이의 대응관계를 저 장해 둘 수 있다. 

다음의 두 가지 목적으로 활용할 수 있다.

1. ﻿﻿﻿순위표에 표시할 사용자 ID와 사용자 객체 사이의 대응관계를 저장한다. (프로필 정보 등 )
   * 이렇게 하면 데이터베이스에 질의하지 않아도 빠르게 사용자 정보를 확인 할 수 있다.
2. ﻿﻿﻿두 사용자의 점수가 같은 경우 누가 먼저 점수를 받았는지에 따라 순위를 매길 수 있다. 사용자 ID와 해당 사용자가 마지막으로 승리한 경기의 타임 스탬프 사이의 대응관계를 저장해 두는 것이다. 동점자가 나오면 기록된 타임스탬프 값이 오래된 사용자 순위가 높다고 하면 된다.

### 시스템 장애 복구

레디스에도 장애는 발생할 수 있으므로, MySQL 데이터베이스에 해당 사실을 기록함으로써 간단히 복구할 수 있다. 

# 11장 결제 시스템

사람들마다 결제 시스템에 대해 무엇을 생각하는지 다르므로 정확한 요구사항을 파악해야 한다

* 어떤 결제 시스템을 만들어야하나요?
* 전자상거래 애플리케이션을 위한 결제 백엔드를 구축해야 함.
* 실생활에서 사용 가능한 모든 옵션 지원. 신용카드, 페이팔, 은행 카드
  * 신용카드 결제 처리는 결제 서비스 업체 사용
* 신용카드 데이터 정보는 민감하므로 결제 처리 업체 의존
* 통화는 달러 하나만 사용
* 하루에 100만건의 거래가 이루어짐.
* 매월 판매자에게 대금을 지급하는 절차 필요

기능 요구사항

- ﻿﻿대금 수신(pay-in) 흐름: 결제 시스템이 판매자를 대신하여 고객으로부터 대 금을 수령한다.
- ﻿﻿대금 정산(pay-out) 흐름: 결제 시스템이 전 세계의 판매자에게 제품 판매 대금을 송금한다.

비기능 요구사항

- ﻿﻿신뢰성 및 내결함성: 결제 실패는 신중하게 처리해야 한다.
- ﻿﻿내부 서비스(결제 시스템, 회계 시스템)와 외부 서비스(결제 서비스 제공업 체) 간의 조정 프로세스: 시스템 간의 결제 정보가 일치하는지 **비동기적으로** 확인한다.
  - 후보정 조정 작업을 뒤에서 실행해야 함.



하루 백만건 / 86400 = 대략 10tps. 일반 데이터베이스로 처리 가능한 양.

**처리 대역폭 대신, 트랜잭션의 정확한 처리에 초점을 맞춰야 함.**

## 2단계 : 개략적 설계안 제시 및 동의 구하기

구매자가 주문을 하면 전자상거래 사이트의 계좌로 돈이 들어오고, 배송이 완료되고 나면 그제서야 판매 대금이 판매자에게 넘어간다.

### 결제 서비스

결제 서비스는 결제 이벤트를 수락하고 결제프로세스를 조율한다.

![image-20250311135348264](./images//image-20250311135348264.png)

자금 세탁, 테러자금 조달과같은 범죄 행위인지 여부를 판단하는 위험 점검이 필요한데 매우 고도화 되어있기 때문에 제3자 제공업체를 이용한다.

### 결제 실행자

payment executor는 PSP를 통해 결제 order 하나를 실행한다. 하나의 결제 이벤트에는 여러 결제 주문이 포함될 수 있따.

### 결제 서비스 공급자 - PSP

PSP는 돈을 인출해서 옮기는 역할을 한다.
...
1. 주문하기 버튼 클릭시 결제 이벤트 생성되어 결제 서비스로 전송
2. 결제 서비스는 결제 이벤트를 데이터베이스에 저장
3. 때로는 단일 결제 이벤트에 여러 결제 주문이 포함될 수 있음.
4. 결제 실행자(payment executor)는 결제 주문을 데이터베이스에 저장
5. 결제 실행자가 외부 PSP 호출하여 신용카드 결제 처리
6. 결제 실행자가 결제 성공 처리시 결제 서비스는 지갑을 갱신하여 특정 판매자의 잔고 기록
...
```json
{
  "buyer_info": {},  // 구매자 정보 (JSON 형식)
  "checkout_id": "",  // 해당 결제 이벤트를 식별하는 전역적으로 고유한 ID (string)
  "credit_card_info": {},  // 암호화된 신용 카드 정보 또는 결제 토큰. PSP마다 다른 값. (JSON 형식)
  "payment_orders": [
    {
      "seller_account": "",  // 대금을 수령할 판매자 (string)
      "amount": "",  // 해당 주문으로 전송되어야 할 대금 (string)
      "currency": "",  // 주문에 사용된 통화 단위 (ISO 4217, string)
...
* payment_order_id는 전역적으로 고유한 id이며, 결제 실행자가 psp에 결제 요청시 idempotency key로 사용한다.

* 또한 amount(금액)은 string으로 처리해야 한다. 매우 클수도 있고, double은 시스템에 따라 가격이 달라질 수 있기 때문 

GET /v1/payemnt/{:id}

* payment_order_id가 가리키는 단일 결제 주문의 시랭 상태를 반환한다.

### 결제 서비스 데이터 모델

결제 서비스에는 결제 event와 결제 order의 두개 테이블이 아니다.

성능 대신 다음 사항에 중점을 둔다.

1. ﻿﻿﻿안정성이 검중되었는가? 즉, 다른 대형 금융 회사에서 수년(가령 5년 이상) 동안 긍정적인 피드백을 받으며 사용된 적이 있는가?
2. ﻿﻿﻿모니터링 및 데이터 탐사(investigation)에 필요한 도구가 풍부하게 지원되 는가?
3. ﻿﻿﻿데이터베이스 관리자(DBA) 채용 시장이 성숙했는가? 다시 말해 숙련된 DBA를 쉽게 채용할 수 있는가? 아주 중요하게 고려해 봐야 할 요소다.

NoSQL보다는 ,ACID 트랜잭션을 지원하는 전통적인 RDB를 선호한다.
...
* checkout_id 는 fk다. 
* 구매자의 신용 카드에서 금액을 공제하기 위해 타사 PSP를 호출하면 판매자 대신 전자상거래 웹사이트의 은행 계좌에 이체가 이루어지는데 이것을 대금 수신(pay-in)이라 한다. 이후 판매자에게 이체된다. 따라서 사용자의 결제를 처리하는 중에는 판매자의 은행 계좌가 아닌, 구매자의 카드 정보만 필요하다.



payment_order_status는 결제 주문의 실행 상태이다.

* NOT_STARTED_EXECUTING_SUCCESS_FAILED 등이 있다.



1. payment_order_status의 초깃값은 NOT_STARTED다.

2. 결제 서비스는 결제 실행자에 주문을 전송하면 payment_order_status의 값을 EXECUTING로 바꾼다.

3. 결제 서비스는 결제 처리자의 응답에 따라 payment_order_status의 값을 SUCCESS 또는 FAIL로 변경한다.

SUCCESS로 성공하고 나면, 결제 서비스는 지갑 서비스를 호출하여 판매자 잔액을 업데이트 하고, wallet_updated 필드의 값은 TRUE로 업데이트 한다.

이후 결제 서비스는 원장 서비스를 호출하여 원장 DB의 ledger_updated 필드를 true로 갱신한다. 

이후, 종결되지 않은 결제 주문을 모니터링 하기 위해 주기적으로 scheduling job을 마련해둔다.

### 복식부기 원장 시스템 - double-entry

모든 결제 시스템에 필수 요소이며, 정확한 기록을 남기는데 역할을 한다.

모든 결제 거래를 두개의 별도 원장 계쫘에 같은 금액으로 기록한다.

한 계좌에서는 차감이 이루어지고, 다른 계좌에는 입금이 동시에 이루어진다.

복식부기에서 모든 거래 항목의 합계는 0이어야 한다. 

### 외부 결제 페이지

신용카드 정보를 대부분의 기업은 내부에 저장하지 않는다. 복잡한 법규 때문이다.

때문에 신용카드 정보를 취급하지 않는 기업들은 PSP에서 제공하는 외부 신용카드 페이지를 사용한다.

웹사이트의 경우 widget 또는 iframe이며, 모바일 애플리케이션의 경우 결제 SDK에 포함되어있다.

## 3단계 상세 설계

### PSP 연동

대부분의 회사는 다음 2가지 방법중 하나로 결제 시스템을 PSP와 연동한다.

1. 직접 신용카드 정보 관리하는 경우 API를 통해 PSP와 연동한다. 
2. 카드 결제 세부 정보를 수집하여 PSP에 안전하게 저장할 수 있또록 외부 결제 페이지를 제공한다. 

![image-20250311144821231](./images//image-20250311144821231.png)

1. 사용자가 클라이언트 브라우저에서 결제 버튼 클릭하고, 클라이언트는 결제 주문 정보를 담아 결제 서비스 호출
2. 결제 주문 정보를 수신한 결제 서비스는 결제 등록 요청을 PSP에 전송, 금액, 만료일, 리디렉션 URL등 정보가 포함되고, 결제 등록이 1번만 등록될 수 있도록 UUID 필드를 둔다. 
3. PSP는 요청한 결제 서비스에 토큰을 반환하며, 등록된 결제 요청을 유일하게 식별할 수 있는 uuid다
4. 결제 서비스는 PSP가 제공하는 외부 결제 페이지를 호출하기 전에 토큰을 데이터베이스에 저장한다
5. 토큰을 저장하고 나면 클라이언트는 PSP가 제공하는 외부 결제 페이지를 표시한다. 모바일 애플리케이션은 일반적으로 이를 위해 PSP SDK를 연동한다. 외부 결제 페이지는 다음 두가지 정보를 필요로 한다
   1. 4단계에서 받은 토큰 : 이 토큰을 사용하여 PSP의 백엔드에서 결제 요청에 대한 상세 정보를 검색한다
   2. 리디렉션 URI : 결제 완료시 호출될 웹 페이지 URL이다. 
6. 사용자는 카드번호 등 결제 세부정보를 PSP의 웹 페이지에 입력 후 결제를 하고 PSP가 결제를 시작한다
7. PSP가 결제 결과를 반환한다
8. 사용자는 리디렉션 URL을 가리키는 웹 페이지로 보내지며, 7단계에서 수신된 결제 상태가 URL에 쿼리 파라미터로 추가된다
9. 비동기적으로 PSP는 웹 훅을 통해 결제 상태와 함께 결제 서비스를 호출한다. 웹 훅은 결제 시스템측에서 PSP를 설정할때 등록한 URL이며, 결제 시스템이 웹훅을 통해 결제 이벤트를 수신하면 결제 상태를 추출하여 결제 주문 데이터베이스의 status 필드를 업데이트한다.

만약 위 아홉단계 각각이 네트워크 문제로 실패한다면? 장애는 어떻게 처리할까. 

### 조정  (reconciliation)

관련 서비스간의 상태를 주기적으로 비교하여 일치하는지 확인해야한다.

매일 밤 PSP나 은행은 고객에세 정산 파일을 보낸다. 정산 파일에는 하루동안 해당 계좌에서 발생한 모든 거래내역이 있따.

![image-20250311145305802](./images//image-20250311145305802.png)

조정 시스템은 정산 파일의 세부 정보를 읽어 원장 시스템과 비교한다.

이후, 조정 중에 발견된 차이는 재무팀에 의뢰하여 수동으로 고친다.

발생 가능한 불일치 문제 및 해결방안

1. 어떤 유형의 문제인지 알고 있으며 해결 절차를 자동화 할 수 있는 경우 : 이경우 엔지니어가 자동화한다
2. 어떤 유형의 문제인지는 알지만, 자동화 할 수는 없는 경우 : 작업 대기열에 넣고 재무팀에서 수동으로 수정
3. 분류할 수 없는 문제 유형의 경우 : 특별 작업 대기열에 넣고, 재무팀에서 조사한다

### 결제 지연 처리

구매 페이지가 외부 PSP에 호스팅 되어있는 경우

- ﻿﻿PSP는 결제가 대기(pending) 상태임을 알리는 상태 정보를 클라이언트에 반환하고, 클라이언트는 이를 사용자에게 표시한다. 클라이언트는 또한 고 객이 현재 결제 상태를 확인할 수 있는 페이지도 제공한다.
- ﻿﻿PSP는 우리 회사를 대신하여 대기 중인 결제의 진행 상황을 추적하고, 상태 가 바뀌면 PSP에 등록된 웹훅을 통해 결제 서비스에 알린다.

어떤 PSP는 결제 요청의 상태를 polling 하도록 하기도 한다 
...
* 단일 수신자 : 각 메시지는 하나의 수신자 또는 서비스가 처리한다.
  * 일반적으로 공유 메시지 큐를 사용해 구현. 
* 다중 수신자 : 여러 수신자 또는 서버가 처리. 카프카같은 시나리오
  * 하나의 요청이 푸시 알림, 재무 업데이트, 분석 결과 업데이트 등에 쓸 수 있음.
...
* 결제 상태는 데이터 추가가 가능한 데이터베이스 테이블에 보관한다

재시도 큐 및 실패 메시지 큐

![image-20250311150056319](./images//image-20250311150056319.png)

* 재시도 가능한 오류는 재시도 큐에 보냄
* 데드레터 큐 : 메시지를 디버깅하고 격리하여 이유를 파악하기 위한 검사에 사용 

### '정확히 한 번(exact-once)' 전달

언뜻 보기에 메시지를 정확히 한 번 전달하는 것은 매우 어려운 문제처럼 느 껴지지만, 문제를 두 부분으로 나누면 훨씬 쉽게 해결할 수 있다. 수학적으로 보자면, 다음의 요건이 충족되면 주어진 연산은 정확히 한 번 실행된다.
...
재시도 메커니즘 사용시 어떤 결제가 최소 한 번은 실행되도록 보장 가능하다.

재시도 간격 전략

- ﻿﻿즉시 재시도(immediate retry): 클라이언트는 즉시 요청을 다시 보낸다.
- ﻿﻿고정 간격(fixed interval): 재시도 전에 일정 시간 기다리는 방안이다.
- ﻿﻿증분 간격(incremental interval): 재시도 전에 기다리는 시간을 특정한 양 만 큼 점진적으로 늘려 나가는 방안이다.
- ﻿﻿지수적 백오프(exponential backoft) 7: 재시도 전에 기다리는 시간을 직전 재시도 대비 두 배씩 늘려 나가는 방안. 예를 들어, 요청이 처음 실패하면 1 초 후에 재시도하고, 두 번째로 실패하면 2초, 세 번째로 실패하면 4초를 기 다린 후 재시도한다.
- ﻿﻿취소(cancel): 요청을 철회하는 방안. 실패가 영구적이거나 재시도를 하더 라도 성공 가능성이 낮은 경우에 흔히 사용되는 방안이다.

재시도시 발생할 수 있는 문제는 이중 결제다

1. 클라이언트가 결제 버튼 따닥
2. 결제는 성공적으로 PSP가 처리하였으나, 응답이 결제 시스템에 도착하지 못함.

이중 결제를 방지하려면 결제는 '최대 한 번' 이루어져야 한다. '최대 한 번 실 행'은 다른 말로 떡등성(idempotency)이라고도 부른다.
...
시나리오 1 : 고객이 결제 버튼을 빠르게 두번 클릭하는 경우

* 두번째 요청의 경우 이전 결제 요청의 가장 최근 상태를 반환
  * 동일한 멱등키로 동시에 많은 요청을 받으면 하나만 처리하고 나머지는 429 상태코드 반환
  * 데이터베이스 unique key 조건을 활용한다. 
    * 결제 요청을 받으면 디비에 새 레코드를 넣으려 시도한다
    * 추가에 성공시 이전에 처리한 적 없는 요청. 
    * 실패한것은 이전에 받은 적이 있는 결제 요청 

시나리오 2 : PSP가 결제를 성공적으로 처리했지만, 네트워크 오류로 응답이 결제 시스템에 전달되지 못하여 사용자가 다시 클릭하는경우

* 사용자가 결제 버튼을 다시 누른다 해도 결제 주문이 같으니 PSP로 전송되는 난수도 같으므로, 이전 실행 결과 반환

### 일관성

결제 실행 과정에서 상태 정보를 유지 관리하는 여러 서비스가 호출된다.

1. ﻿﻿﻿결제 서비스는 비중복 난수, 토큰, 결제 주문, 실행 상태 등의 결제 관련 데 이터를 유지 관리한다.
...
내부 서비스간에 일관성을 유지하려면 요청이 정확히 한번 처리되도록 보장해야 한다.

내부서비스와 외부서비스간 데이터 일고나성 유지시 멱등성과 조정 프로세스를 활용한다.

데이터를 다중화 하는경우 복제 지연으로 인해 원본과 사본이 불일치 할 수 있다. 이경우 두가지 해결 방법이 있따.

1. 주 데이터베이스에서만 읽기-쓰기 연산 처리. 사본은 데이터 백업용
2. 모든 사본이 항상 동기화 되도록 한다. 레프트 같은 합의 알고리즘 사용 

## 4단계 마무리:

- ﻿﻿모니터링: 주요 지표를 모니터링 하는 것은 모든 현대적 애플리케이션에 아 주 중요하다. 광범위한 모니터링을 통해 "특정 결제 수단의 평균 승인율은 얼마인가?", "서버의 CPU 사용량은 얼마인가?" 같은 질문에 답을 얻을 수 있 다. 지표들을 한데 모아 대시보드를 만들 수도 있다.
- ﻿﻿경보: 비정상적인 상황이 발생하면 온콜(on-call) 중인 개발자에게 알려 신 속하게 대응할 수 있도록 하는 것이 중요하다.
- ﻿﻿디버깅 도구: "왜 결제가 실패하나요?"는 아주 흔한 질문이다. 엔지니어와 고객 지원팀이 더 쉽게 디버깅할 수 있도록 결제 거래의 상태, 처리 서버 기 록, PSP 기록 등을 검토할 수 있는 도구를 개발하는 것이 중요하다.
- ﻿﻿환율: 국제적인 결제 시스템을 설계할 때 환율은 중요한 고려사항이다.
- ﻿﻿지역: 지역마다 가용한 결제 수단이 완전히 달라질 수 있다.
- ﻿﻿현금 결제: 현금 결제는 인도, 브라질 등의 국가에서 매우 일반적이다. 우버 와 에어비앤비가 현금 결제 방법에 대해 자세한 엔지니어링 블로그 기사를 펴낸 바 있으니, 참고하기 바란다.

*  구글/애플 페이 연동: [30]을 읽어 보기 바란다.

# 12장 전자 지갑

전자지갑은 고객이 돈을 넣어둘 수 있꼬, 지갑끼리 송급이 가능하다. 

지갑간 이체를 지원하는 전자 지갑 애플리케이션의 백엔드를 설계해보자 

## 1단계 문제 이해 및 설계 범위 확정

* 두 전자 지갑 사이의 이체에만 집중해야 하나요, 다른 기능도 신경써야 하나요
  * 이체 기능에만 집중
* 시스템이 지원해야 하는 초당 트랜잭션 수 TPS는 얼마인가요
  * 1,000,000 TPS
* 전자 지갑의 대한 정확성은 데이터베이스가 제공하는 트랜잭션 보증이면 충분한지?
  * 충분
* 정확성을 증명해야 하나요?
  * 내부 기록과 은행 명세서를 비교하는 조정 작업으로는 일관성이 깨졌다는것은 알 수 있지만 차이는 모름
  * 때문에 재현성(reproducibility)를 갖춘 시스템을 설해보자
* 가용성 요구사항은 99.99%

### 개략적 추정

오늘날 일반적인 RDB는 초당 수천건의 트랜잭션 지원 가능. 

대략적으로 데이터베이스 노드당 1,000 TPS라고 가정하고, 1백만 TPS 이므로 데이터베이스는 1천개가 필요함. 

* 그러나, 인출 -> 입금 시스템이므로 2배인 초당 2천 TPS가  필요함

1천개의 DB는 하드웨어 비용이 많아지므로, 설계 목표 중요 한가지는 단일 노드의 처리 트랜잭션 량을 늘리는것.
...
* 3가지 개략적 설계안
  ...
  3. 재현성을 갖춘 event sourcing 솔루션

### 인메모리 샤딩

레디스 노드 한대로 1백만 TPS는 벅참. 클러스터를 구성하고 사용자를 모든 노드에 균등하게 분산시켜야 함.

키-값 데이터를 n개 파티션에 분배하려면, 키의 해시값을 계산하고 이를 파티션 수로 나누는것이 한가지 방법.

모든 레디스 노드 및 파티션 수 및 주소는 한군데인 zookeper를 두면 좋다.

<img src="./images//image-20250317212033917.png" width = 550>

### 분산 트랜잭션

#### 데이터베이스 샤딩

서로 다른 두 개 저장소 노드를 갱신하는 연산을 원자적으로 어떻게 수행할까?

첫번째 단계는 각 레디스 노드를 트랜잭션을 지원하는 관계형 데이터베이스 노드로 교체한다.

<img src="./images//image-20250317212137610.png" width = 450>

3개의 관계형 데이터베이스 노드로 분산되어 있는데, 두 데이터베이스의 트랜잭션을 어떻게 보장할까?

### 분산 트랜잭션 - 2PC

저수준 방안으로 사용되는 알고리즘이다. 2단계 커밋이다.

![image-20250317212633886](./images//image-20250317212633886.png)

1. ﻿﻿﻿조정자(이번 설계안의 경우에는 지갑 서비스)는 정상적으로 여러 데이터베 이스에 읽기 및 쓰기 작업을 수행한다. 그림 12.5와 같이, 그 결과로 데이터 베이스 A와 C에는 락이 걸린다.

2. ﻿﻿﻿애플리케이션이 트랜잭션을 커밋하려 할 때 조정자는 모든 데이터베이스 에 트랜잭션 준비를 요청한다.

3. ﻿﻿﻿두 번째 단계에서 조정자는 모든 데이터베이스의 응답을 받아 다음 절차를 수행한다.

   1. ﻿﻿﻿모든 데이터베이스가 '예'라고 응답하면 조정자는 모든 데이터베이스 에 해당 트랜잭션 커밋을 요청한다.

   1. ﻿﻿﻿어느 한 데이터베이스라도 '아니요'를 응답하면 조정자는 모든 데이터 베이스에 트랜잭션 중단을 요청한다.



가장 큰 문제점은, 

1. 모든 데이터베이스가 X/Open XA 표준을 만족해야 하며
2. 다른 노드의 메시지를 기다리는 도안 락이 오랫동안 잠김 상태로 남을 수 있어 성능이 좋지 않음.
3. 그리고 조정자가 SPOF 즉 단일 장애 지점이 될 수 있음

### 분산 트랜잭션 : TC/C

TC/C(시도-확정/취소, try-confirm/cancel)은 두단계로 구성된 보상 트랜잭션이다.

1. ﻿﻿﻿조정자는 모든 데이터베이스에 트랜잭션에 필요한 자원 예약을 요청한다.

2. ﻿﻿﻿조정자는 모든 데이터베이스로부터 회신을 받는다.

   1. ﻿﻿﻿﻿모두 '예'라고 응답하면 조정자는 모든 데이터베이스에 작업 확인을 요 청하는데, 이것이 바로'시도- 확정(TryContfimm) 절차다.

   1. ﻿﻿﻿﻿어느 하나라도 '아니요'라고 응답하면 조정자는 모든 데이터베이스에 작업 취소를 요청하며, 이것이 바로 '시도-취소(Try-Cancel)' 절차다.

### 분산 트랜잭션 : 사가

사가의 개념

1. ﻿﻿﻿모든 연산은 순서대로 정렬된다. 각 연산은 자기 데이터베이스에 독립 트 랜잭션으로 실행된다.
2. ﻿﻿﻿연산은 첫 번째부터 마지막까지 순서대로 실행된다. 한 연산이 완료되면 다음 연산이 개시된다.

3. 연산이 실패하면 전체 프로세스는 실패한 연산부터 맨 처음 연산까지 역순 으로 보상 트랜잭션을 통해 롤백된다. 따라서 개의 연산을 실행하는 분산 트랜잭션은, 보상 트랜잭션을 위한 r개 연산까지 총 2n개의 연산을 준비해 야 한다.



연산 실행 순서를 구현하는법은 2가지가 있다.

1. ﻿﻿﻿분산 조율(Choreography, 즉 '안무'라고도 함): 마이크로서비스 아키텍처 에서 사가 분산 트랜잭션에 관련된 모든 서비스가 다른 서비스의 이벤트를 구독하여 작업을 수행하는 방식. 완전히 탈 중앙화된 조율 방식이다.
2. ﻿﻿﻿중앙 집중형 조율(Orchestration): 하나의 조정자(coordinator)가 모든 서비 스가 올바른 순서로 작업을 실행하도록 조율한다.



일반적으로 오케스트레이션 방식을 선호한다. 복잡한 상황을 잘 처리하기 때문이다
...
이 둘 가운데, latancy 요구사항에 따라 선택하면 된다.

TC/C는 병렬로 실행할 수 있지만 사가는 순서대로 실행해야 한다.

1. ﻿﻿﻿지연 시간 요구사항이 없거나 앞서 살펴본 송금 사례처럼 서비스 수가 매 우 적다면 아무것이나 사용하면 된다. 마이크로서비스 아키텍처에서 흔히 하는 대로 하고 싶다면 사가를 선택하면 된다.
2. ﻿﻿﻿지연 시간에 민감하고 많은 서비스/운영이 관계된 시스템이라면 TC/C가 더 낫다.

### 이벤트 소싱

실제로 전자 지갑 서비스 제공 업체도 감사를 받을 수 있다. 예를 들어 외부 감 사(auditor)는 다음과 같은 까다로운 질문들을 던질 수 있다.

1. ﻿﻿﻿특정 시점의 계정 잔액을 알 수 있나요?
2. ﻿﻿﻿과거 및 현재 계정 잔액이 정확한지 어떻게 알 수 있나요?
3. ﻿﻿﻿코드 변경 후에도 시스템 로직이 올바른지는 어떻게 검증하나요?

이러한 질문에 체계적으로 답할 수 있는 설계 철학 중 하나는 도메인 주도 설 계(Domain-Driven Design, DDD)에서 개발된 기법인 이벤트 소싱이다.
...
명령 : 외부에서 전달된 의도가 명확한 요청. A->C 이체하라는 요청은 명령. 순서는 아주 중요하므로 명령은 일반적으로 FIFO큐에 저장

이벤트 : 명령 이행 결과를 이벤트라고 부른다. 작업 이행 전 반드시 명령의 유효성을 검사해야 한다. 명령과 이벤트 사이에는 두 차이점이 있따.

1. ﻿﻿﻿이벤트는 검증된 사실로, 실행이 끝난 상태다. 그래서 이벤트에 대해 이야 기할 때는 과거 시제를 사용한다. 따라서 명령이 "A에서 C로 1 송금"인 경 우, 해당 이벤트는 "A에서 C로 1 송금을 완료하였음"이 된다.
2. ﻿﻿﻿명령에는 무작위성(randomness)이나 I/O가 포함될 수 있지만 이벤트는 결 정론적(deterministic)이다. 이벤트는 과거에 실제로 있었던 일이다.

* 이벤트 생성 프로세스의 두가지 중요한 특성 
  * 하나의 명령으로 여러 이벤트가 만들어 질 수 있음.
  * 이벤트 생성 과정에 무작위성이 개입될 수 있어서 같은 명령에 항상 동일한 이벤트 들이 만들어진다는 보장이 없음. 
* 이벤트 순서는 명령 순서를 따라야 하므로 FIFO 큐에 저장한다

상태 : 이벤트가 적용될때 변경되는 내용. 지갑 시스템에서 상태는 클라이언트 계정의 잔액이다 .

상태 기계 : 이벤트 소싱 프로세스를 구동한다. 크게 두가지 기능이 있다.
...
명령은 FIFO 큐에 기록하며, 카프카를 사용한다.

<img src="./images//image-20250317233826146.png" width= 350>
...
![image-20250317234510154](./images//image-20250317234510154.png)

#### 재현성 (reproducibility)

*재현성(Reproducibility)**은 동일한 입력과 환경에서 동일한 결과를 다시 얻을 수 있는 성질을 의미. 즉, 같은 조건에서 동일한 과정을 반복했을 때 **항상 같은 결과가 나오는 것**을 보장하는 개념.

이벤트 소싱의 가장 중요한 장점은 재현성이다.

재현성을 갖추면 감사관(auditor)이 던지는 까다로운 질문에 쉽게 답할 수 있 다. 
...
첫 번째 질문의 경우, 시작부터 계정 잔액을 알고 싶은 시점까지 이벤트를 재 생하면 알 수 있다.

두 번째 질문의 경우, 이벤트 이력에서 계정 잔액을 다시 계산해보면 잔액이 정확한지 확인할 수 있다.

세 번째 질문의 경우, 새로운 코드에 동일한 이벤트 이력을 입력으로 주고 같 은 결과가 나오는지 보면 된다.

감사 가능 시스템이어야 한다는 요건 때문에 이벤트 소싱이 지갑 서비스 구 현의 실질적인 솔루션으로 채택되는 경우가 많다.

---

지원자: 이벤트 소싱 아키텍처를 사용하면 전체 시스템에 재현성을 확보할 수 있습니다. 모든 유효한 사업 기록은 변경이 불가능한 이벤트 대기열에 저장되어, 정확성 검증에 사용될 수 있습니다.

면접관: 훌륭하군요. 하지만 제안하신 이벤트 소싱 아키텍처는 한 번에 하나의

이벤트만 처리하는 데다 여러 외부 시스템과 통신해야 합니다. 더 빠 르게 만들 수 있을까요?

## 3단계 상세 설계

### 고성능 이벤트 소싱

카프카를 명령 및 이벤트 저장소로, DB를 상태 저장소로 사용했다. 가능한 최적화 방안을 살펴보자



이전 설계안에서는 잔액 정보를 RDB에 저장했다.

그러나 최적화 방안으로 상태 정보도 로컬 디스크에 저장할 수 있다. 

파일 기반 로컬 RDB인 SQLite나 로컬 파일 기반 키 -값 RocksDB를 사용 가능하다.

RocksDB는 LSM을 사용하기 때문에 쓰기 작업에 최적화되어있다.

---

지원자: 명령 목록, 이벤트 목록, 상태, 스냅숏이 모두 파일에 저장되도록 이벤

트 소싱 아키텍처를 변경할 수 있습니다. 이벤트 소싱 아키텍처는 애

초에 이벤트 목록을 선형적으로 처리하므로 HDD나 운영체제 캐시와

궁합이 잘 맞습니다.



면접관: 로컬 파일 기반 솔루션의 성능은 원격 카프카나 데이터베이스에 저장

된 데이터를 액세스하는 시스템보다는 좋다고 할 수 있습니다. 하지만

로컬 디스크에 데이터를 저장하는 서버는 더 이상 무상태(stateless) 서

버가 아닌데다, 단일 장애 지점이 된다는 문제가 있습니다. 시스템의

안정성은 어떻게 개선할 수 있을까요?

### 합의

높은 안정성을 제공하려면 여러 노드에 복제해야 한다. 복제 과정은 다음을 보장해야 한다. 

1. ﻿﻿﻿데이터 손실 없음
2. ﻿﻿﻿로그 파일 내 데이터의 상대적 순서는 모든 노드에 동일

합의 기반 복제 알고리즘인 Raft 알고리즘을 보자

Raft 알고리즘 사용시 노드 절반 이상이 온라인 상태면,그 모두에 보관된 append-only 리스트는 같은 데이터는 다진다. 

* ex 5개 노드가 있을때 3개만 온라인이여도 전체 시스템은 정상 동작
...
하나의 노드만 클러스터의 리더가 되고 나머지 노드는 팔로워가 된다.

팔로어 장애는 처리하기가 훨씬 쉽다. 팔로어에 장애가 생기면 해당 팔로어 로 전송된 요청은 실패한다. 래프트는 죽은 노드가 다시 시작되거나 새로운 노 드로 대체될 때까지 기한 없이 재시도하여 해당 장애를 처리한다.
지원자: 래프트 합의 알고리즘을 사용하여 여러 노드에 이벤트 목록을 복제하
였습니다. 명령 수신과 복제는 리더가 담당합니다.
면접관: 네, 시스템의 안정성과 결함 내성이 향상되었네요. 하지만 100만 TPS 를 처리하려면 서버 한 대로는 충분하지 않습니다. 어떻게 하면 시스 템의 확장성을 높일 수 있을까요?
...
1. 전자 지갑 업데이트 결과는 즉시 받고 싶다. 하지만 CQRS 시스템에서는 요 청/응답 흐름이 느릴 수 있다. 클라이언트가 디지털 지갑의 업데이트 시점 을 정확히 알 수 없어서 주기적 폴링(polling)에 의존해야 할 수 있기 때문 이다.
2. 단일 래프트 그룹의 용량은 제한되어 있다. 일정 규모 이상에서는 데이터 를 샤딩하고 분산 트랜잭션을 구현해야 한다.
...
* 증권 거래시, 주식만 거래하고 새 주문, 주문 취소, 지정가 주문만 가능
* 특정 시간 내 거래만 처리 가능 
* 거래소가 갖추어야 하는 기본 기능은 무엇인가요?
  * 주문 접수, 기존 주문 취소, 주문 체결시 실시간 체결사실 확인
  * 호가 창은 실시간 갱신 되어야 함.
  * 최소 수만명 사용자가 동시 거래 가능. 최소 100가지 주식 
  * 수십억 건의 주문도 발생 가능
  * 위험성 점검(risk check)도 가능해야함
...
* 가용성 : 최소 99.99% 
* 결함 내성 : 빠른 복구 메커니즘 필요
* 지연 시간 : p99 지연 시간이 매우 빨라야함
* 보안 : 사용자 신원 확인을 위한 KYC, 공개 자원의 경우 DDOS 방지

### 개략적 규모 추정

* 100가지 주식
* 하루 10억건의 주문 
* 뉴욕 증권 거래소 예시로 하루 영업시간은 6.5시간 
* 10억 / 6.5 시간 x 3600 = 42735 tps
* 최대 qps : 5 x qps = 215,000. 피크 거래도 계싼해야함

## 2단계 : 개략적 설계안 제시 및 동의 구하기

### 거래소 용어 정리

* 브로커 : 개인 고객은 브로커를 통해 거래소와 거래한다. (로빈후드, 하나증권 등 증권사). 브로커 시스템은 개인 사용자가 증권을 거래하고 시장 데이터를 확인할 수 있도록 편리한 사용자 인터페이스 제공
* 기관 고객 : 전문 증권 소프트웨어를 사용하여 대량으로 거래함. 거래 빈도는 낮지만 거래량은 많음. 대규모 주문이 시장에 미치는 영향을 최소화하기 위해 주문 분할 같은 기능을 필요로 함. 
* 지정가 주문 : 가격이 고정된 매수 또는 매도 주문. 시장과 주문과는 달리 체결이 즉시 이루어지지 않을 수 있고, 부분적으로만 체결될수도 있음
* 시장가 주문 : 가격을 지정하지 않는 주문으로 시장가로 즉시 체결. 
* 시장 데이터 수준 : 미국 주식시장은 L1, L2, L3의 가격 정보 등급이 있다.
  * L1 : 시장 데이터에는 최고 매수 호가, 매도 호가, 수량이 포함 
  * L2 : 체결을 기다리는 물량의 호가를 어디까지 보여주는지 나타냄
  * L3 : 주문 가격에 체결을 기다리는 물량 정보까지 나타냄 
* 봉 차트 : 캔들. 시작가, 종가, 최저가 표시 가능. 보통 1분, 5분, 1시간, 1일, 1주일, 1개월
* FIX : 금융 정보 교환 프로토콜. 

### 개략적 설계안

![image-20250319194329713](./images//image-20250319194329713.png)
...
4. 주문 관리자가 위험 관리자가 설정한 규칙에 따라 점검 수행
5. 위험성 점검 과정을 통과한 주문에 대해, 주문 관리자는 지갑에 주문 가능한지 확인
6. 주문이 체결 엔진으로 전송. 체결가능 주문 발생시 체결 엔진은 매수측과 매도측에 하나씩 두개의 실행 기록을 남김.
7. 클라이언트한테 주문 사실을 전송

시장 데이터 흐름은, 하나의 주문이 체결 엔진부터 데이터 서비스를 거쳐 브로커로 전달되어 집행되기까지의 과정이다.

* M1단계 : 체결 엔진은 주문 체결시 집행 기록 스트림을 만들고, 이 스트림은 시장 데이터 개시 서비스로 전송
* M2 단계 : 시장 데이터 게시 서비스는 집행 기록 및 주문 스트림에서 얻은 데이터를 시장 데이터로 사용하며 봉 차트와 호가 창을 구성
* M3 단계 : 시장 데이터는 실시간 분석 전용 스토리지에 저장되고, 브로커는 데이터 서비스를 통해 실시간 시장 데이터를 읽음



보고 흐름. 보고 서비스는 주문 및 실행기록에서 보고에 필요한 모든 필드의 값을 모아 레코드를 데이터베이스에 기록

### 체결 엔진

체결 엔진의 주요 역할

1. order book 호가창 유지 관리. 
2. 매수 주문과 매도 주문 연결. 
...
체결 엔진에 주문 전달 전 sequence를 붙여 보낸다. 처리를 끝낸 주문도 sequence를 붙여보낸다.

입력되는 주문과 출력하는 실행 명령에 순서 ID를 찍는 이유는 다음과 같다.

1. ﻿﻿﻿시의성(timeliness) 및 공정성(faimness)
...
한쪽에서는 주문을 받고, 한쪽에서는 집행 기록을 받아 주문 상태를 관리함.

### API 설계

RESTful API로는 헤지펀드와 같은 기관 고객의 지연 시간 요구사항을 충족 하지 못할 수도 있다. 이러한 기관에 공급되는 특수 소프트웨어는 다른 프로토 콜을 사용할 가능성이 높다.
...
주로 UI 표시를 위한 데이터이며, 아무 데이터베이스에나 저장 가능하고 캐시 적용하기 좋다

주문과 집행 기록은 가장 중요한 데이터이다.
...
평균 지연시간은 낮아야 하고 전반적인 지연시간 분포는 안정적이여야 한다.

지연시간을 줄이는 2가지 방법

1. 중요 경로에서 실행할 작업 수를 줄임
2. 각 작업의 소요 시간을 줄임
   1. 네트워크 및 디스크 사용량
   2. 각 작업의 실행 시간 경감
...
네트워크랑 디스크 지연량을 모두 고려하면 수십 밀리초임. 

더 낮추려면 모든것을 동일한 서버에 배치하여 네트워크를 통하는 구간을 없애는것. 

같은 서버 내 컴포넌트간 통신은 이벤트 저장소인 mmap을 통한다. 

<img src="./images//image-20250319201212068.png" width = 350>

애플리케이션 루프를 만들어 while을 통해 실행할 작업을 계속 폴링하고 단일 스레드로 구현해서 특정 cpu 코어에 고정시킨다.

<img src="./images//image-20250319201452310.png" width = 450>

이렇게 했을때의 장점

1. ﻿﻿﻿문맥 전환, 즉 컨텍스트 스위치(context switch)가 없다. 8) CPU 1이 주문 관 리자의 애플리케이션 루프 처리에 온전히 할당된다.
2. ﻿﻿﻿상태를 업데이트하는 스레드가 하나뿐이라서 락을 사용할 필요가 없고 잠 금 경합(lock contention)도 없다.



##### mmap 

mmap은 파일을 프로세스의 메모리에 매핑하는 POSIX 호환 UNIX 시스템 콜이다.

프로세스간 고성능 메모리 공유 메커니즘을 제공한다. 

mmap을 활용하여 만든 이벤트 저장소에 이벤트 소싱 패러다임을 결합하면 서버에 지연시간이 낮은 마이크로 서비스 구축이 가능하다

#### 이벤트 소싱

![image-20250319203155049](./images//image-20250319203155049.png)

mmap 이벤트 저장소를 메시지 버스로 사용한다.

*  게이트웨이는 빠르고 간결한 인코딩을 위해 FIX를 SBE(FIX over Simple Bi-nary Encoding)로 변환하고, 각 주문을 이벤트 저장소 클라이언트를 사용하여 미리 정의된 형식의 NeworderEvent 형태로 전송한다(다이어그램에서는 이벤트 저장소 항목) 
* ﻿﻿체결 엔진에 내장된 주문 관리자는 이벤트 저장소로부터 NeworderEvent를 수신하면 유효성을 검사한 다음 내부 주문 상태에 추가한다. 그 후 해당 주 문은 처리 담당 CPU 코어로 전송된다.
* ﻿﻿주문이 체결되면 OrderFiLLedEVent가 생성되어 이벤트 저장소로 전송 된다.
* ﻿﻿시장 데이터 프로세서 및 보고 서비스 같은 다른 구성요소는 이벤트 저장소 를 구독하고, 이벤트를 받을 때마다 적절히 처리한다.



### 고가용성

99.99% 가용성은 장애 시간이 8.64초를 넘으면 안된다는것이다.



- ﻿﻿첫째로, 거래소 아키텍처의 단일 장애 지점(SPOF)을 식별해야 한다. 예를 들어 체결 엔진에 발생하는 장애는 거래소에는 재앙이다. 따라서 주 인스턴 스를 다중화해야 한다.
- ﻿﻿둘째, 장애 감지 및 백업 인스턴스로의 장애 조치 결정이 빨라야 한다.

위 설계안의 문제점은 단일 서버 안에서만 동작하므로, UDP를 통해 모든 부 서버에 브로드 캐스트 한다.

### 결함내성

결함 내성(fault-tolerant)

시스템을 만들려면 많은 질문에 답해야 한다.
...
체결 엔진의 체결 결과를 받아 이를 기반으로 호가 창과 봉 차트를 재 구축한 다음 구독자에게 데이터를 publish

MDP의 메모리는 무한대로 확장할 수 없으므로 봉 차트에는 상한선을 두어야 한다.



링 버퍼(원형 버퍼)에 데이터를 생산자를 넣고 소비자는 데이터를 꺼내는 방식으로 처리한다.

### 멀티캐스트

인터넷에서 데이터는 세 가지 유형의 프로토콜을 통해 전송된다.

1. ﻿﻿﻿유니캐스트(unicast): 하나의 출처에서 하나의 목적지로만 보내는 전송 프 로토콜이다.
2. ﻿﻿﻿브로드캐스트(broadcast): 하나의 출처에서 전체 하위 네트워크로 보내는 방식이다.
3. ﻿﻿﻿멀티캐스트(multicast): 하나의 출처에서 다양한 하위 네트워크상의 호스트 들로 보내는 방식이다.

거래소 설계에 보편적으로 이용되는 것은 멀티캐스트다. 같은 멀티캐스트 그 룹에 속한 수신자는 이론적으로는 동시에 데이터를 수신한다. 그러나 UDP는 신뢰성이 낮은 프로토콜이며 그 데이터그램(datagram)은 모든 수신자에게 도 달하지 못할 수도 있다. 

* 재전송 방법 : https://en.wikipedia.org/wiki/NACK-Oriented_Reliable_Multicast