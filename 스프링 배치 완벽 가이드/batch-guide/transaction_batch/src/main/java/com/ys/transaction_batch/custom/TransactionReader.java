...
public class TransactionReader implements ItemStreamReader<Transaction> {
	...
	public TransactionReader(final ItemStreamReader<FieldSet> fieldSetReader) {
	...
	private Transaction process(FieldSet fieldSet) {
		if (this.recordCount == 25) {
			...
			throw new ParseException("이건 내가 바라던 일이 아니야");
		}
		...
		if (fieldSet != null) {
			if (fieldSet.getFieldCount() > 1) {
				...
				recordCount++;
			} else { // 레코드에 필드가 하나만 있으면 footer 레코드 (푸터 레코드는 마지막 레코드를 의미한다)
